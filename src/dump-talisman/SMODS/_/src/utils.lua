LOVELY_INTEGRITY = '559f3a48ec183053bd2419ccf5f18b204cd1e08a711c62bc7b1d35cdbb6eab23'
...
function SMODS.get_enhancements(card, extra_only)
    ...
    local cached = SMODS.enh_cache:read(card, extra_only)
...
SMODS.enh_cache = {
    ...
    read = function(self, key, extra_only)
        ...
        if not extra_only then return d end
        -- extra_only strips the card's own key from the result; must copy to
        -- avoid mutating the cached table.
        local ret = copy_table(d)
        ret[key.config.center.key] = nil
        return ret
    end,
    clear = function(self)
        self.data = setmetatable({}, _enh_weak_mt)
    end,
}

LOVELY_INTEGRITY = 'ad71c805dc595ea2fefb5b413cd8ae2babda6f20344df77915a18de1463d3590'
...
function Game:update(dt)
    ...
    if G.SETTINGS.paused then dt = 0 end
        ...
        local _sin_dark = math.sin(self.TIMERS.REAL*1.3)
        self.C.DARK_EDITION[1] = 0.6+0.2*_sin_dark
        self.C.DARK_EDITION[3] = 0.6+0.2*(1-_sin_dark)
        self.C.DARK_EDITION[2] = math.min(self.C.DARK_EDITION[3], self.C.DARK_EDITION[1])

        local _ed_base = self.TIMERS.REAL*1.5
        self.C.EDITION[1] = 0.7+0.2*(1+math.sin(_ed_base + 0))
        self.C.EDITION[3] = 0.7+0.2*(1+math.sin(_ed_base + 3))
        self.C.EDITION[2] = 0.7+0.2*(1+math.sin(_ed_base + 6))
        ...
        if not self._exp_dt or math.abs(self.real_dt - self._exp_dt) > 5e-4 then
            self._exp_dt = self.real_dt
...
function Game:draw()
    ...
    -- Precompute the frame-constant part of the screen_scale shader uniform.
    -- G.TILESCALE, G.TILESIZE, and G.CANV_SCALE only change on resize/orientation
    -- events, never mid-frame.  Caching here avoids ~4000 redundant multiplications
    -- and :send() calls per frame (one per draw_shader invocation).
    G._DRAW_SCREEN_SCALE = G.TILESCALE * G.TILESIZE * G.CANV_SCALE
    --draw the room
    reset_drawhash()

...
function Cryptid.gameset_config_UI(center)
	...
	if center.set == "Content Set" then
		...
		-- Only show toggle button if the set has items to toggle
		local set_items = cry_items_for_set(center.key)
		if #set_items > 0 then
			args.back3 = true
...
local function cry_set_item_override(key, enabled)
	if enabled then
...
local function cry_items_for_set(set_key)
	local items = {}
...
local function cry_item_display_name(v)
	...
	for i = base, math.min(base + CRY_TOGGLES_PER_PAGE - 1, #items) do
		...
		nodes[#nodes + 1] = create_toggle({
			...
			callback = function(new_val)
				cry_set_item_override(v.key, new_val)
...
function Cryptid.item_toggle_UI(content_set)
	...
	local items = cry_items_for_set(content_set.key)
	...
	local initial_toggles = UIBox({
		definition = {
			n = G.UIT.ROOT,
			config = { align = "tl", padding = 0.05, colour = G.C.CLEAR },
			nodes = cry_build_toggle_page(items, 1, item_states),
		},

...
function create_UIBox_generic_options(args)
	...
	-- back3: third optional button (used for per-item toggle screen)
	if args.back3 then
		local mainUI = ret.nodes[1].nodes[1].nodes
		mainUI[#mainUI + 1] = {
			n = G.UIT.R,
			config = {
				id = args.back3_id or "overlay_menu_back3_button",
				align = "cm",
				minw = 2.5,
				padding = 0.1,
				r = 0.1,
				hover = true,
				colour = args.back3_colour or G.C.ORANGE,
				button = args.back3_func or "exit_overlay_menu",
				shadow = true,
				focus_args = { nav = "wide", button = "b", snap_to = args.snap_back3 },
			},
			nodes = {
				{
					n = G.UIT.R,
					...
					nodes = {
						{
							n = G.UIT.T,
							config = {
								text = args.back3_label or localize("b_back"),
								scale = 0.5,
								colour = G.C.UI.TEXT_LIGHT,
								shadow = true,
							},

...
function Card:set_cost_value()
    if self.edition then
        ...
        if G.GAME.modifiers.cry_no_edition_price then break end
    ...
    if self.ability.rental and (not (self.ability.set == "Planet" and next(SMODS.find_card("j_astronomer"))) and self.ability.set ~= "Booster") then self.cost = 1 end
end
...
function Card:add_to_deck(from_debuff)
    ...
    if not self.added_to_deck then
        ...
        if not from_debuff and G.hand then
            local is_playing_card = SMODS.is_playing_card(self)
...
function Card:get_nominal(mod)
    ...
    local no_suit, no_rank = SMODS.has_no_suit(self), SMODS.has_no_rank(self)
    if self.ability.effect == 'Stone Card' or (no_suit and no_rank) then
        mult = -10000
    elseif no_suit then
        mult = 0
    elseif no_rank then
...
function Card:use_consumeable(area, copier)
    ...
    if self.ability.rental and not copier then
    	G.E_MANAGER:add_event(Event({
    		trigger = 'immediate',
    		blocking = false,
    		blockable = false,
    ...
    local gone = false
    if self.ability.banana and not copier then
...
function Card:check_use()
    if self.area == G.pack_cards and G.pack_cards and booster_obj and self:selectable_from_pack(booster_obj) then return end
    if self.ability.name == 'Ankh' then 
            ...
            G.E_MANAGER:add_event(Event({trigger = 'after', delay = 1.3*math.sqrt(G.SETTINGS.GAMESPEED), blockable = false, blocking = false, func = function()
                ...
                for i = 1, _size do
                    ...
                    if flags.booster_create_flags then
                        flags.booster_create_flags.skip_materialize = true
...
function Card:highlight(is_higlighted)
    ...
    if self.ability.consumeable or self.ability.set == 'Joker' then
        if not self.highlighted and self.area and self.area.config.type == 'joker' and
            self.config.center_key ~= 'c_cry_delete' and

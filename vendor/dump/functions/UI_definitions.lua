        ...
        for k, v in ipairs(G.GAME.tags) do
          ...
          for _, v in ipairs(rates) do
            if polled_rate > check_rate and polled_rate <= check_rate + v.val then
              local args = {set = v.type, area = area, key_append = 'sho', front = false}
              local flags = SMODS.calculate_context({create_shop_card = true, set = v.type, cardarea = area})
          ...
          args.AT = UIBox{
            ...
            definition = 
              {n=G.UIT.ROOT, config = {align = args.cover_align or 'cm', minw = (args.cover and args.cover.T.w or 0.001) + (args.cover_padding or 0), minh = (args.cover and args.cover.T.h or 0.001) + (args.cover_padding or 0), padding = 0.03, r = 0.1, emboss = args.emboss, colour = args.cover_colour}, nodes={
                {n=G.UIT.O, config={draw_layer = 1, object = DynaText({scale = args.scale, string = args.text, maxw = args.maxw, colours = {args.colour},float = true, shadow = true, silent = not args.noisy, args.scale, pop_in = 0, pop_in_rate = 6, rotate = args.rotate or nil, font = args.font})}},
              }}, 
  ...
  function G.UIDEF.card_h_popup(card)
    if card.ability_UIBox_table then
      ...
      if AUT.multi_box then
          ...
          local prototype_obj = card.config.center or card.config.tag
          if prototype_obj and prototype_obj.pools then
              for k, v in pairs(SMODS.ObjectTypes) do
                  if prototype_obj.pools[v.key] and type(v.set_badges) == "function" then
    ...
    local temp_col2 = G.C.DYN_UI.BOSS_DARK
            ...
            {n=G.UIT.R, config={align = "cm"}, nodes={
              {n=G.UIT.C, config={id = 'hud_ante',align = "cm", padding = 0.05, minw = 1.45, minh = 1, colour = temp_col, emboss = 0.05, r = 0.1}, nodes={
                ...
                {n=G.UIT.R, config={align = "cm", r = 0.1, minw = 1.2, minh = 0.666, colour = temp_col2}, nodes={
...
function create_UIBox_blind_choice(type, run_info)
  ...
  G.GAME.orbital_choices[to_number(G.GAME.round_resets.ante)] = G.GAME.orbital_choices[to_number(G.GAME.round_resets.ante)] or {}

  if Talisman.big_ante.has() or not G.GAME.orbital_choices[to_number(G.GAME.round_resets.ante)][type] then 
    local _poker_hands = {}
    for k, v in pairs(G.GAME.hands) do
        if SMODS.is_poker_hand_visible(k) then _poker_hands[#_poker_hands+1] = k end
    end

    G.GAME.orbital_choices[to_number(G.GAME.round_resets.ante)][type] = pseudorandom_element(_poker_hands, pseudoseed('orbital'))
  end
        ...
        {n=G.UIT.R, config={align = "cm", padding = 0.05}, nodes={
          {n=G.UIT.R, config={id = 'blind_desc', align = "cm", padding = 0.05}, nodes={
            ...
            {n=G.UIT.R, config={align = "cm",r = 0.1, padding = 0.05, minw = 3.1, colour = G.C.BLACK, emboss = 0.05}, nodes={
              ...
              {n=G.UIT.R, config={align = "cm", minh = 0.6, maxw = 2.75}, nodes={
              	{n=G.UIT.O, config={w=0.5,h=0.5, colour = G.C.BLUE, object = stake_sprite, hover = true, can_collide = false}},
              	{n=G.UIT.B, config={h=0.1,w=0.1}},

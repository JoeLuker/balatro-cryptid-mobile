...
function mod_chips(_chips)
  ...
  if _chips ~= hand_chips then SMODS.Scoring_Parameters.chips:modify(nil, _chips - (hand_chips or 0)) end
  return _chips
end

function mod_mult(_mult)
  if _mult ~= mult then SMODS.Scoring_Parameters.mult:modify(nil, _mult - (mult or 0)) end
  return _mult
end
...
function set_deck_win()
  if G.GAME.selected_back and G.GAME.selected_back.effect and G.GAME.selected_back.effect.center and G.GAME.selected_back.effect.center.key then
    local deck_key = G.GAME.selected_back.effect.center.key
    if not G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key] then G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key] = convert_usage_entry{count = 1, order = G.GAME.selected_back.effect.center.order, wins = {}, losses = {}, wins_by_key = {}, losses_by_key = {}} end
    if G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key] then

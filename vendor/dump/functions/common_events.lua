...
local rarity = _rarity or SMODS.poll_rarity("Joker", 'rarity'..G.GAME.round_resets.ante..(_append or ''))
        ...
        if G.GAME.oldbpfactor and G.GAME.oldbpfactor >= 2 then
        	if _type == 'Joker' and (_rarity == nil or type(_rarity) ~= "string") and not _legendary and not (G.GAME.used_jokers["j_blueprint"] and not SMODS.showman("j_blueprint")) then
...
function reset_blinds()
    ...
    if G.GAME.round_resets.blind_states.Boss == 'Defeated' then
        ...
        if G.GAME.modifiers.cry_no_small_blind then G.GAME.round_resets.blind_states.Small = 'Hide' end
        G.GAME.round_resets.blind_states.Big = 'Upcoming'
        G.GAME.round_resets.blind_states.Boss = 'Upcoming'
        G.GAME.blind_on_deck = 'Small'
        if G.GAME.modifiers.cry_no_small_blind then G.GAME.blind_on_deck = 'Big' end
   ...
   --Fill all remaining info if this is the main desc
    if not ((specific_vars and not specific_vars.sticker) and (card_type == 'Default' or card_type == 'Enhanced')) then
        ...
        elseif desc_nodes ~= full_UI_table.main and not desc_nodes.name then
            desc_nodes.name = localize{type = 'name_text', key = name_override or _c.key, set = name_override and 'Other' or _c.set, vars = specific_vars or _c.name_vars or _c.vars} 
            ...
            if not desc_nodes.name_styled then
              ...
              localize{type = 'name', key = key, set = set, nodes = desc_nodes.name_styled, fixed_scale = 0.63, no_pop_in = true, no_shadow = true, y_offset = 0, no_spacing = true, no_bump = true, vars = (_c.create_fake_card and _c.loc_vars and (_c:loc_vars({}, _c:create_fake_card()) or {}).vars) or _c.name_vars or _c.vars or {colours = {}}} 
    ...
    SMODS.compat_0_9_8.generate_UIBox_ability_table_card = nil
        if card and card.config and G.GAME.cry_banished_keys[card.config.center_key] and card.area and card.area.config.collection then
            info_queue[#info_queue+1] = { set = "Other", key = "cry_banned_card", }
        end

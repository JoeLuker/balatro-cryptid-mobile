...
G.FUNCS.flame_handler = function(e)
  ...
  for k, v in pairs(G.ARGS.flame_handler) do
    if e.config.id == v.id then 
      ...
      _F.intensity = clamp_bignum(_F.intensity, Talisman.flame_effect.max)
      _F.timer = _F.timer + G.real_dt * math.min(1 + _F.intensity*0.2, Talisman.flame_effect.dt_max)
      
      if _F.intensity == 0 and not _F.less then
      	_F.less = true
      	if _F.timer > 60 * Talisman.flame_effect.dt_max then _F.timer = 1 end
      	if _F.real_intensity > Talisman.flame_effect.decay then
      		_F.real_intensity = Talisman.flame_effect.decay
      		_F.intensity_vel = 0
      		_F.change = 0
      	end
...
(G.STATE == G.STATES.SMODS_BOOSTER_OPENED or G.STATE == G.STATES.PLANET_PACK or G.STATE == G.STATES.STANDARD_PACK or G.STATE == G.STATES.BUFFOON_PACK or (G.hand  )) then 
  ...
  G.FUNCS.use_card = function(e, mute, nosave)
    ...
    local nc
    local select_to = not e.config.SMODS_use_card and card.area == G.pack_cards and G.pack_cards and booster_obj and card:selectable_from_pack(booster_obj)
    if card.ability.consumeable and not select_to then
        ...
        if not nc and card.ability.cry_multiuse and (card.ability.cry_multiuse > 1) then
      ...
      G.TAROT_INTERRUPT = nil
        ...
        func = function()
            ...
            func = function()
                ...
                if G.cry_muse then
                	G.cry_muse = nil
                	card.ability.cry_no_sell_value = true
                	card:set_cost()
                	delay(0.4)

...
EMU=emulator-5560
# REF is the vanilla Balatro SELECTING_HAND ground truth captured by test/ref.sh at 3840x2160.
# Run `test/ref.sh` to regenerate it if vanilla Balatro.love is updated.
REF=/tmp/refshot1.png
OUT=/tmp/${TAG}.png

cd "$RB"
./gradlew :app:assembleDebug -q 2>&1 | tail -5
adb -s "$EMU" install -r app/build/outputs/apk/debug/app-debug.apk >/dev/null 2>&1
# Force the display to the REF resolution (portrait 2160x3840 → landscape 3840x2160). The room
...
print(f"[{tag}] FULL {mask.mean()*100:.1f}% differ | HUD(left25%) {mask[:,:int(W*0.25)].mean()*100:.1f}% | cards(25-90%) {mask[:,int(W*0.25):int(W*0.90)].mean()*100:.1f}%")
# full-frame heat map + side-by-side of the HUD region (left 30%)
heat=(r*0.3).astype('uint8'); heat[mask]=[255,0,255]
def sc(im,h=540): return im.resize((int(im.width*h/im.height),h),Image.LANCZOS)
Image.fromarray(heat).resize((960,540),Image.LANCZOS).save(f'/tmp/{tag}_heat.png')
x1=int(W*0.30)
mine=sc(Image.fromarray(m[:,:x1].astype('uint8'))); ref=sc(Image.fromarray(r[:,:x1].astype('uint8'))); hc=sc(Image.fromarray(heat[:,:x1]))
combo=Image.new('RGB',(mine.width+ref.width+hc.width+20,540),(20,20,20))
...
print(f"  -> /tmp/{tag}_heat.png  (full-frame heat map)")
print(f"  -> /tmp/{tag}_compare.png  (MINE | REF | HEAT of left 30%)")
PY